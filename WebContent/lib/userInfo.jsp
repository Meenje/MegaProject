...
<%@page import="org.json.simple.JSONObject"%>
		...
		<%
	      ...
	      /* MemberDTO dto1 = dao.userinfo(dto); */
...
<body>
	<form name="signUpForm" class="form" role="form">
		<div style="margin: 10px;">
			<h2>마이페이지</h2>
			아이디 : <input type="text" name="id" class="form-control input-lg" id="memberid" placeholder="아이디" value = '<%=dto1.getId()%>' readonly>
			<div id="iddiv"></div><br>
			이름 : <input type="text" name="name" class="form-control input-lg" id="name" placeholder="이름" value = '<%=dto1.getName()%>'><br>
			전화번호 : <input type="text" name="tel" class="form-control input-lg" id="tel" required="required" placeholder="전화번호" value = '<%=dto1.getTel()%>'><br>
			이메일 : <input type="text" name="email" class="form-control input-lg" id="email" placeholder="이메일" value = '<%=dto1.getEmail()%>'><br>
			현재 비밀번호 : <input id="pw" type="password" name="pw" class="form-control input-lg" placeholder="현재 패스워드">
			<div id="pwdiv"></div><br>
			변경할 비밀번호 : <input type="password" name="confirm_password" id="confirm" class="form-control input-lg" placeholder="변경할 패스워드" >
			...
			<input type="radio" name="gender" value="female"
				<%if (dto1.getGender().equals("female")){ %>		
					checked <% } %>> 여자
			<br>
			<section>
				<fieldset>
					<div class="field moveindate">
						...
						min="1900-01-01" max="2019-12-31" spellcheck="false" value = '<%=dto1.getBirth()%>'>
					</div>
				</fieldset>
			</section>
			...
			<button class="btn btn-lg btn-primary btn-block signup-btn"	type="button" id="updatebtn">회원정보수정</button>
			<button class="btn btn-lg btn-primary btn-block signup-btn"	type="button" id="deletebtn">탈퇴</button>
		</div>
	</form>
</body>
